...
def processar_google_sheets(planilha_key):
    try:
        ...
        for aba in planilha.worksheets():
            ...
            logging.info(f"📂 Lendo aba: {titulo_aba}...")
            
            try:
                dados = aba.get_all_values()
            except Exception as e:
                if "Unable to parse range" in str(e):
                    logging.warning(f"⚠️ Erro de nome na aba '{titulo_aba}'. Tentando contornar...")
                    dados = planilha.values_get(f"'{titulo_aba}'!A1:Z1000").get('values', [])
                else:
                    raise e
            
            if not dados or len(dados) < 2:
                logging.warning(f"⚠️ Aba '{titulo_aba}' vazia ou sem dados suficientes. Pulando...")
                continue
            ...
            logging.info(f"🚀 Processando {len(df)} clientes válidos da aba {titulo_aba}...")
        ...
        cursor.close()
        conn.close()
        logging.info("✅ Todas as abas foram processadas com sucesso!")
...
def job():
    ...
    if not ID_DA_PLANILHA:
        logging.error("❌ A variável GOOGLE_SHEET_ID não foi encontrada no .env!")
        return
